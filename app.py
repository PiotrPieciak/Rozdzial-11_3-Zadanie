...
## tab3 callback - bar 
@app.callback(Output('bar-chanel','figure'),
    [Input('chanel-range','start_date'),Input('chanel-range','end_date')])
def tab3_bar_chanel(start_date,end_date):
    ...
    dict_week = dict(enumerate(['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday']))
...
## tab3 callback - dropdown
@app.callback(Output('barh-chanel-subcat','figure'),
            [Input('chanel_dropdown','value')])
def tab3_barh_chanel_subcat(chosen_cat_chan):

    grouped = df.merged[(df.merged['total_amt']>0)&(df.merged['Store_type']==chosen_cat_chan)].pivot_table(index='prod_subcat',columns='Gender',values='total_amt',aggfunc='sum').assign(_sum=lambda x: x['F']+x['M']).sort_values(by='_sum').round(2)
